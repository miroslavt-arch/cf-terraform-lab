...
`root-a/` and `root-b/` both target the `http_request_late_transform` phase
(chosen so the demo never touches the real WAF phase or the Topic 29 legacy
phase). Run: `scripts/demo-32-phase.sh`
